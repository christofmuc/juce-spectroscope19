...
Use `setXAxis(true)` for logarithmic frequency mapping and `setHorizontalMode(true)` for horizontal history. `setPitchColourMode(true)` keeps the physical FFT energy in greyscale and overlays circle-of-fifths colour only for temporally tracked tonal peaks. `setTrackedNoteOverlayEnabled(true)` adds a diagnostic note/cents/confidence list. `setConcertAHz()` controls the shared pitch-analysis and display reference.
