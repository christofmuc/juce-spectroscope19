...
- **Tracked notes** annotates each inferred fundamental on the frequency axis. The three rows show note name, signed cents relative to the current A4 reference, and confidence; for example `F2`, `+14.0 ct`, and `70%`. In horizontal-history mode the same information becomes three columns at the note's vertical frequency position. Released annotations fade smoothly, and overlapping annotations are painted from least to most confident so the stable note remains readable.
